...
view shared transitions { testCase, wasCorrect } =
    let
        recognitionSpecResult =
            PLL.getUniqueTwoSidedRecognitionSpecification
                { pllAlgorithmUsed = Debug.log "algorithm" (User.getPLLAlgorithm (PLLTrainer.TestCase.pll testCase) shared.user |> Maybe.withDefault Algorithm.empty)
    ...
    , body =
        case recognitionSpecResult of
            Err _ ->
                View.fullScreenBody (always none)
