...
function addToDocumentHead(toAdd: string, body: string) {
...
function parseSendToApp(javascriptString: string) {
  ...
  const sendToAppDefinition = javascriptString.substring(startIndex, endIndex);
...
function parseTheJavascript(javascriptString: string) {
  ...
  while (
    !(
      startLevel === -1 &&
      javascriptString.substr(initializeStartIndex, "function".length) ===
        "function"
    )
  ) {
